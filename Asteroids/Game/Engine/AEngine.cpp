...
void AEngine::SetCameraCenter(const CCPoint &cell)
{
    if (!_map) {
        return;
    }
...
void AEngine::SetMap(shared_ptr<MAXContentMap> map)
{
	if (_map)
	{
		delete _map;
...
void AEngine::AddResourceCell(const int x, const int y, const RESOURCE_TYPE type, const unsigned char amount)
{
    if (!_resourceRenderer)
        return;
...
void AEngine::AddFogCell(const int x, const int y, const bool fog)
{
    if (!_fogRenderer) 
        return;
...
void AEngine::FillFog()
{
    _fogRenderer->CompletlyFillMap();
}

void AEngine::ClearResourceFog()
{
    _resourceRenderer->Clear();
}

void AEngine::AddPathZoneCell(const int x, const int y)
{
	_pathZoneRenderer->AddCell(x, y);
}

void AEngine::ClearPathZone()
{
	_pathZoneRenderer->Clear();
}

void AEngine::AddOptionalZoneCell(const int x, const int y)
{
	_optionalZoneRenderer->AddCell(x, y);
}

void AEngine::ClearOptionalZone()
{
    _optionalZoneRenderer->Clear();
}

void AEngine::SetOptionalZoneColor(const Color &c)
{
...
void AEngine::DrawGrid()
{
    if(drawGrid)
        _grid->DrawGrid();
}

void AEngine::DrawGround()
{
    DEBUG_FUNCTION_ENTER(this);
    _shader = _mapShader;
...
void AEngine::applicationDidEnterBackground() {
    
}

bool AEngine::applicationDidFinishLaunching() {
    return true;
}

void AEngine::applicationWillEnterForeground() {
